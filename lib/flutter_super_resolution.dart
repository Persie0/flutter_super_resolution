...
class FlutterUpscaler {
  ...
  Future<ui.Image?> upscaleImage(
    ...
    try {
      if (!useTiling || (sourceImage.width <= tileSize && sourceImage.height <= tileSize)) {
  ...
  Future<OrtValueTensor> _prepareInputTensor(ui.Image image, int width, int height) async {
  ...
  Future<List<dynamic>> _runInference(OrtValueTensor inputTensor) async {
    ...
    try {
      final outputs = await _session!.runAsync(runOptions, {'input': inputTensor});
